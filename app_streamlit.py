...
print(len(tables))  # Muestra cuántas tablas encontró
...
# Verificar
print(demo["RIDSTATR"].value_counts())
...
print(bpq['Diagnóstico de hipertensión (Sí/No)'].unique())
...
# Verificar cambios
print(bpq['Diagnóstico de hipertensión (Sí/No)'].head(10))
...
# Ver tamaño final
print("Tamaño final de df", df.shape)
...
print("Valores únicos objetivo:", df["Diagnóstico de diabetes (Sí/No)"].unique())
print("NaN en objetivo:", df["Diagnóstico de diabetes (Sí/No)"].isna().sum())
...
print(f"Número de variables numéricas: {num_numericas}")
print(f"Número de variables categóricas: {num_categoricas}")
...
# Mostrar resultados
print("=== Estadísticas descriptivas para todas las variables numéricas ===\n")
print(estadisticas)
...
print("Columnas que se eliminarán (más del 80% de valores faltantes):")
...
print("Nombres de las variables en df:")
for col in df.columns:
    print("-", col)
...
print(f"[PCA] Componentes seleccionados: {k_pca} (umbral={VAR_THRESHOLD:.0%})")
print("[PCA] Varianza explicada acumulada (primeras 10):", np.round(var_cum[:10], 4))
...
print("Guardado pca_componentes.csv con shape:", DF_PCA_final.shape)
...
print("Tamaño matriz disyuntiva:", X_disc.shape)
...
print(f"Dimensiones seleccionadas (MCA): {k_mca} (≥ {VAR_THRESHOLD:.0%})")
print("Inercia acumulada (primeras 10):", np.round(inertia_cum[:10], 4))
...
print("✔ Guardado: mca_dimensiones.csv |", DF_MCA_out.shape)
...
print("✔ Guardado: tarea1_pca_mca_concat.csv |", DF_final.shape)
...
print("Filas, Columnas:", df_final.shape)             # -> (6072, 18)
print("#PCs:", len(pc_cols), "| #DIMs:", len(dim_cols), "| #Objetivo:", len(y_cols))
...
print(f"[RF] {len(orig_sel_rf)} variables originales seleccionadas  → tarea2_variables_seleccionadas_RF.csv")
...
print(f"[L1] {len(orig_sel_l1)} variables originales seleccionadas → tarea2_variables_seleccionadas_L1.csv")
...
print(f"AUC 5-fold | RF vars: {auc_rf:.4f}")
print(f"AUC 5-fold | L1 vars: {auc_l1:.4f}")

DF_sel_final = DF_sel_RF if auc_rf >= auc_l1 else DF_sel_L1
print("Ganador:", "RF" if auc_rf >= auc_l1 else "L1")
...
print(f"AUC 5-fold | PCA+MCA:        {auc_pca_mca:.4f}")
print(f"AUC 5-fold | Sel. variables: {auc_final:.4f}")
